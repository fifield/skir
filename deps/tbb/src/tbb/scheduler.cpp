...
task* generic_scheduler::local_find_and_steal_task( task &target ) {

    for (unsigned i=0; i<my_arena->my_limit; i++) {
	arena_slot& victim_slot = my_arena->slot[i];
	task** victim_pool = lock_task_pool( &victim_slot );
	if ( !victim_pool )
	    continue;
	const size_t none = ~size_t(0);
	size_t first_skipped_mismatch = none;
	task* result = NULL;
    retry:
	++victim_slot.head;
	atomic_fence();
	if ( (intptr_t)victim_slot.head > (intptr_t)victim_slot.tail ) {
	    ...
	    __TBB_ASSERT( !is_poisoned(result), NULL );
	    if( &target != result ) {
                if ( first_skipped_mismatch == none )
                    first_skipped_mismatch = victim_slot.head - 1;
                result = NULL;
                goto retry;
            }
	...
	if ( first_skipped_mismatch != none ) {
	    if ( result ) {
		victim_pool[victim_slot.head - 1] = victim_pool[first_skipped_mismatch];
		poison_pointer( victim_pool[first_skipped_mismatch] );
		__TBB_store_with_release( victim_slot.head, first_skipped_mismatch + 1 );
	    }
	    else
		__TBB_store_with_release( victim_slot.head, first_skipped_mismatch );
	}
	if (result && is_proxy(*result)) {
	    result = strip_proxy((task_proxy*)result);
	}
	...
	if (result) {
	    result->prefix().state = task::allocated;
	    return result;
	}
    }
    return 0;
