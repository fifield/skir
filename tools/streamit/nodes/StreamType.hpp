 ...
 */
#ifndef _STREAMTYPE_HPP_
#define _STREAMTYPE_HPP_
 ...
 */
class StreamType : public FENode
{
private:
     Type *in, *out, *loop;
...
public:
     ...
     */
    StreamType(FEContext *context, Type *in, Type *out)
	: FENode(context), in(in), out(out), loop(in)
    {
    }
     ...
     */
    StreamType(FEContext *context, Type *in, Type *out, Type *loop)
	: FENode(context), in(in), out(out), loop(loop)
    {
    }
